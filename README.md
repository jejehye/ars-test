...
2·3·4·5·9번의 모든 하위 정상·오류 후보·상담원 시나리오은 대메뉴 직후 인증하고 중·소메뉴를 선택한다. PASS/FAIL/BLOCKED, 메모, CSV, 브라우저 저장을 유지한다.
...
1-5는 계좌 인증을 지원하며 종목코드는 사용하지 않는다. 3-1-1 ARS 주문은 종목코드 자동/직접입력 선택을 제공한다.

Android 기본값을 full로 변경했다. 기존 자동 판단 설정은 한 번 full로 전환하며 명시적인 반자동 선택은 유지한다.
